...
class DualEncoders:
    def __init__(self, hparams):
        ...
        losses = tf.nn.sigmoid_cross_entropy_with_logits(logits, tf.to_float(self.targets), name="CrossEntropy")
    ...
    def inference(self):
        ...
        with tf.variable_scope("BidirectionalLSTM"):
            ...
            fw_cell = tf.nn.rnn_cell.LSTMCell(self.hparams.rnn_dim, **argsdict)
            bw_cell = tf.nn.rnn_cell.LSTMCell(self.hparams.rnn_dim, **argsdict)
            _, rnn_states = tf.nn.bidirectional_dynamic_rnn(fw_cell, bw_cell,
                                                            tf.concat(0, [context_emb, utterance_emb]),
                                                            sequence_length=tf.concat(0, [self.context_len,
                                                                                          self.utterance_len]),
                                                            dtype=tf.float32)

            fw_encoding_context, fw_encoding_utter = tf.split(0, 2, rnn_states[0].h)
            bw_encoding_context, bw_encoding_utter = tf.split(0, 2, rnn_states[1].h)

            encoding_context = tf.concat(1, [fw_encoding_context, bw_encoding_context])
            encoding_utterance = tf.concat(1, [fw_encoding_utter, bw_encoding_utter])
    ...
    def validation_accuracy(self, pred_labels, val_labels, val_loss):
        shaped_probs = tf.reshape(pred_labels, [-1, 100])
    ...
    def validate(self, batch_dict):
        val_targets = np.zeros([len(batch_dict["label"]) / 100], dtype=np.int64)
        feed_dict = {self.context: batch_dict["context"],
                     self.context_len: batch_dict["context_len"],
                     self.utterance: batch_dict["utterance"],
                     self.utterance_len: batch_dict["utterance_len"],
